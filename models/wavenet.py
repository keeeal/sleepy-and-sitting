...
class ConstantPad1d(Function):
    ...
    def forward(self, input):
        ...
        if self.pad_start:
            ...
            c_output = c_output.narrow(
                self.dimension, 0, c_output.size(self.dimension) - self.num_pad
            )
    ...
    def backward(self, grad_output):
        ...
        if self.pad_start:
            ...
            cg_output = cg_output.narrow(
                self.dimension, 0, cg_output.size(self.dimension) - self.num_pad
            )
...
class WaveNetModel(nn.Module):
    ...
    Args:
        ...
        num_classes (Int):              Number of possible values each sample can have
    ...
    def __init__(
        ...
        self.start_conv = nn.Conv1d(
            in_channels=3, out_channels=residual_channels, kernel_size=1, bias=bias
        )
        ...
        self.end_conv_2 = nn.Conv1d(
            in_channels=end_channels, out_channels=num_classes, kernel_size=1, bias=True
        )
    ...
    def generate(self, num_samples, first_samples=None, temperature=1.0):
        ...
        for i in range(num_samples):
            input = Variable(
                torch.FloatTensor(1, self.num_classes, self.receptive_field).zero_()
            )
            ...
            if temperature > 0:
                ...
                x = np.random.choice(self.num_classes, p=np_prob)
        ...
        generated = (generated / self.num_classes) * 2.0 - 1
        mu_gen = mu_law_expansion(generated, self.num_classes)
    ...
    def generate_fast(
        ...
        self.eval()
        if first_samples is None:
            first_samples = torch.LongTensor(1).zero_() + (self.num_classes // 2)
        ...
        input = Variable(torch.FloatTensor(1, self.num_classes, 1).zero_())
        ...
        for i in range(num_given_samples - 1):
            ...
            input.zero_()
            input = input.scatter_(
                1, first_samples[i + 1 : i + 2].view(1, -1, 1), 1.0
        ...
        regularizer = torch.pow(
            Variable(torch.arange(self.num_classes)) - self.num_classes / 2.0, 2
        )
        ...
        for i in range(num_samples):
            ...
            if (i + 1) == 100:
                toc = time.time()
                print(
                    "one generating step does take approximately "
                    + str((toc - tic) * 0.01)
                    + " seconds)"
                )
